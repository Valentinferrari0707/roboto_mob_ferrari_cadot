...
from math import fabs, sqrt, atan2, pi, fmod, copysign
...
class LocalPlanner:
    ...
    def __init__(self, K_linear, K_angular, Sat_linear, Sat_angular, Obstacle_range, Angle_to_allow_linear, Waypoint_error, Destination_error, Angle_error):
        ...
        # init ros node
        rospy.init_node('localPlanner_student', anonymous=False)
        ...
        rospy.loginfo("Enter a single goal point with rosservice call /move_to/singleGoal '[x, y, z]' or click on a point to achieve to start")
...
#******************************************************************************************
    ...
    def odomCallback(self, odom):
        ...
        self.curPose2D = Pose2D(x,y,angle)
        # rospy.loginfo("X: %f \t Y: %f \t theta: %f"  %s (self.curPose2D.x, self.curPose2D.y, self.curPose2D.theta ) )
        # print(self.curPose2D)
    ...
    def scanCallback(self, scan):
       ...
       # detect
        minRange = min(scan.ranges)
        if(minRange <= self.Obstacle_range):
            self.isObstacle = True
        else:
            # print(minRange)
            self.isObstacle = False
        if (self.scanCompt == 100):
            if self.isObstacle: 
...
#******************************************************************************************
    ...
    def pathService(self, req) :
        ...
        try:
            now = rospy.Time(0)
            listener.waitForTransform(req.pathToGoal.header.frame_id, "/odom", now, rospy.Duration(5.0))
            for i in xrange( len(req.pathToGoal.poses) ) :
                # print (req.pathToGoal.poses)
            ...
            print("self.pathposes   ", self.pathPoses)
...
#******************************************************************************************
    ...
    def computeDistAngle(self):
        ...
        if len(self.pathPoses) > 0 :
            ...
            # print("compute dist angle!")
            poseWayPoint = self.pathPoses[0]
            poseRobot = self.curPose2D


            Xwp = poseWayPoint.pose.position.x
            Ywp = poseWayPoint.pose.position.y 
            Xr =  poseRobot.x
            Yr =  poseRobot.y
            thetaRobot =  poseRobot.theta
            # angle_error = np.arctan(Ywp/Xwp)
            angle_error = atan2(Ywp-Yr,Xwp-Xr)

            distCurTarget = sqrt((Xwp - Xr)**2+(Ywp - Yr)**2)
            angle = self.shortestAngleDiff(angle_error,thetaRobot)
            # print("distCurTarget and angle:")            
            # print(distCurTarget,angle)
    ...
    def computeFinalOrientation(self):
        ...
        if len(self.pathPoses) > 0 :
            # calculate angle
            orientation = self.pathPoses[0].pose.orientation
            ...
            angleWayPoint  = euler[2]
            angle = self.shortestAngleDiff(angleWayPoint, self.curPose2D.theta)
    ...
    def pathSequencer(self, dist, angle, finalOrientation):
        ...
        if(dist < self.Waypoint_error ) and len(self.pathPoses) > 1 :
            ...
            # print("del self.pathPoses[0] ::::: ", self.pathPoses[0])
            #computeVelocity
    ...
    def computeVelocity(self, dist, angle, goalState) :
        ...
        #TODO for students : Apply gain and saturation (both ROSPARAM) to angle
        #copysign =>  Returns : float value consisting of magnitude from parameter x and the sign from parameter y
        twist.angular.z = min(self.K_angular * abs(angle), self.Sat_angular) * copysign(1, angle)
        ...
        # print("computeVelocity ==> twist  ", twist, self.isObstacle)
        # print(self.isObstacle) 
        return twist   
    ...
    def localPlanning(self):
        ...
        while not rospy.is_shutdown():
            ...
            if "Reach in progress" == goalState: #TODO for students : modify string matching with the state (help in pathSequencer docstring)
                (dist, angle) = self.computeDistAngle()

            elif "Last Goal position Reached" == goalState  or  "Last Goal pose (position + orientation) Reached" == goalState :  #TODO for students : modify string matching with the state (help in pathSequencer docstring)        
                angle = finalOrientation

            # print("before twist  dist and angle  goalState :::::: ", dist, angle, goalState)
...
if __name__ == '__main__':
    try:
        ...
        OBSTACLE_RANGE = rospy.get_param('~OBSTACLE_RANGE', 0.3)
